...
if uploaded_file:
    ...
    st.write("📝 Transkriptionstext (Ausschnitt):", transcript.text[:300])", type=["mp3", "wav", "m4a", "webm"])
if uploaded_file:
    st.success("📥 Datei erfolgreich hochgeladen.")
    with tempfile.NamedTemporaryFile(delete=False, suffix=".webm") as f:
        f.write(uploaded_file.read())
    ...
    os.remove(f.name)
    st.session_state.audio_base64 = None  # Deaktiviert ggf. vorheriges Audio
    st.session_state.transcription_text = transcript.text
    st.session_state.transcription_done = True
    st.audio(uploaded_file, format="audio/webm")
    st.write("📝 Transkriptionstext (Ausschnitt):", transcript.text[:300])

# Statusanzeige vor dem Button
if not audio_ready:
    st.info("ℹ️ Noch keine Audioaufnahme verfügbar. Bitte Aufnahme beenden.")
else:
    st.success("✅ Audioaufnahme empfangen. Transkription kann gestartet werden.")

transcribe_button_clicked = st.button("🧠 Transkription starten", disabled=transcribe_disabled)
